...
class CreateFinancialReleaseUseCaseTest extends UseCaseTest {
  ...
  void givenAValidCommand_whenCallsCreateFinancialRelease_shouldReturnCreateFinancialReleaseId() {
    ...
    Subcategory subcategory = Subcategory.newSubcatergory("Teste", CategoryID.from(2L))
        .with(expectedSubcategoryID);

    when(subcategoryGateway.findById(expectedSubcategoryID)).thenReturn(Optional.of(subcategory));
    ...
    final var command = CreateFinancialReleaseCommand.with(BigDecimal.valueOf(2),

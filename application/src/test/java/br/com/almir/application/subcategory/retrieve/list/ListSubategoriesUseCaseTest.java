...
class ListSubategoriesUseCaseTest extends UseCaseTest {

  @InjectMocks
  private DefaultListSubategoriesUseCase useCase;
  ...
  @Test
  void givenAValidQuery_whenCallsListSubcategories_thenShouldReturnSubcategories() {
    final var categories = List.of(
        Subcategory.newSubcatergory("Carro", CategoryID.from(1L)).with(SubcategoryID.from(2L)),
        Subcategory.newSubcatergory("Farmacia", CategoryID.from(1L)).with(SubcategoryID.from(3L)));
    ...
    final var expectedResult = expectedPagination.map(SubcategoryListOutput::from);

    when(subcategoryGateway.findAll(eq(query)))
  ...
  @Test
  void givenAValidQuery_whenHasNoResults_thenShouldReturnEmptySubcategories() {
    final var categories = List.<Subcategory>of();
